...
class DIOSTEPPER(QtWidgets.QDialog,ui_dio_stepper.Ui_Dialog):
	...
	def setPins(self,v):
		if v==0: #Ap bp an bn
			self.p.ap=1;self.p.bp=2;self.p.an=4;self.p.bn=8;
		elif v==1: #Ap an bp bn
			self.p.ap=1;self.p.an=2;self.p.bp=4;self.p.bn=8;
		self.p.stepper_positions = [self.p.ap+self.p.bp, self.p.an+self.p.bp, self.p.an+self.p.bn, self.p.ap+self.p.bn] # full step sequence. 
